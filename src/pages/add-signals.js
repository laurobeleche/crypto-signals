...
function AddSignals(props) {
  ...
  const addEntry = () => {
    if (signalForm.entradas.length > 0) {
      const minEntry = Math.min(...signalForm.entradas);
      if (parseFloat(signalForm.precoEntrada) >= parseFloat(minEntry)) {
        alert("A nova entrada deve ser menor que as entradas já cadastradas.");
        return;
      }
    }
  ...
  const addAlvo = () => {
    ...
    if (signalForm.palvos.length > 0) {
      const maxAlvo = Math.max(...signalForm.palvos);
      if (parseFloat(signalForm.alvos) <= parseFloat(maxAlvo)) {
        alert("O novo alvo deve ser maior que o maior alvo já cadastrado.");
        return;
      }
    }
